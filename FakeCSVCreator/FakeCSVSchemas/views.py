...
from .forms import FakeCSVSchemeForm, FakeSchemeColumnInline, ExportDatasetForm
from .models import FakeCSVSchemes
...
class MySchemesView(LoginRequiredMixin, ListView):
    queryset = FakeCSVSchemes
    template_name = "schemas/list.html"

    def get_queryset(self):
        return FakeCSVSchemes.objects.filter(author=self.request.user)


class CreateSchemeView(LoginRequiredMixin, CreateWithInlinesView):
    model = FakeCSVSchemes
    form_class = FakeCSVSchemeForm
    template_name = "schemes/create-edit.html"
    inlines = [FakeSchemeColumnInline]
...
class DeleteSchemeView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    model = FakeCSVSchemes
...
class EditSchemeView(LoginRequiredMixin, UserPassesTestMixin, UpdateWithInlinesView):
    model = FakeCSVSchemes
    form_class = FakeCSVSchemeForm
